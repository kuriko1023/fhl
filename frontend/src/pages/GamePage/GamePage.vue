...
<!--            TODO: 是否有效-->
        <view v-if='side === 0' form-type="submit" :class="'btn' + (answerSendTimer !== -1 ? ' disabled' : '')" @tap='onSubmitAnswer'>发送</view>
        </view>
      </form>
    </view>
<!--    临时测试用-->
...
export default {
  ...
  methods:{
    ...
    onEnd(){
      redirect('/pages/EndPage/EndPage')
    },
    ...
    onSocketMessage() {
      if (tryPeekSocketMessage('end_status')) {
        redirect('/pages/EndPage/EndPage');
        return;
      }

      const msg = popSocketMessage(['game_status', 'game_update', 'invalid_answer']);
      switch (msg.type){
        case 'game_status': {
          ...
          for(let i = 0; i < msg.history.length; i++){
            let sentence = historySentenceParse(msg.history[i])
        ...
        case 'game_update':{
          ...
          this.answer = historySentenceParse(msg.text)
