...
<!--        <uni-col :span="12">-->
          <view class="modeChoose">
            <radio-group @change="onModeChange" >
              <view><label><radio :disabled="!isHost" class="theme" value="A" :checked="mode==='A'?true:false"/><text class="radio_text">梦笔生花</text></label></view>
              <view><label><radio :disabled="!isHost" class="theme" value="B" :checked="mode==='B'?true:false"/><text class="radio_text">走马观花</text></label></view>
              <view><label><radio :disabled="!isHost" class="theme" value="C" :checked="mode==='C'?true:false"/><text class="radio_text">天女散花</text></label></view>
              <view><label><radio :disabled="!isHost" class="theme" value="D" :checked="mode==='D'?true:false"/><text class="radio_text">雾里看花</text></label></view>
            </radio-group>
          </view>
<!--        </uni-col>-->
<!--        <uni-col :span="6">-->
...
<!--      </uni-row>-->
      ...
      <view class="bottom">
        <view v-if='isHost'>
          <view>
            <button @tap="generate" class="btn1" :disabled="isGenReqPending">{{isSubject?'换一换':'生成题目'}}</button>
          </view>
          <view>
            <button class="btn2" @tap="onConfirm" :disabled="!isSubject">
              确定
            </button>
          </view>
        </view>
...
export default {
  ...
  methods:{
    ...
    onConfirm(){
      sendSocketMessage({
        type: this.isHost ? 'confirm_subject' : 'confirm_start'
      });
    },
    generate(){
    ...
    onSocketMessage() {
      if (tryPeekSocketMessage('game_status')) {
        redirect('/pages/GamePage/GamePage');
        return;
      }

      const msg = popSocketMessage('generated');
      if (msg._none) return;
      //更新题目，与选择器绑定
